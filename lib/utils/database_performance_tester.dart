...
class DatabasePerformanceTester {
  ...
  List<Project> _generateTestProjects(int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      projects.add(Project(
        ...
        notes: 'Test notes for project ${i + 1}',
      ));
    }
  ...
  List<Studio> _generateTestStudios(int count) {
    ...
    final studioTypes = [StudioType.recording, StudioType.production, StudioType.hybrid];

    for (int i = 0; i < count; i++) {
      final type = studioTypes[_random.nextInt(studioTypes.length)];
  ...
  List<Booking> _generateTestBookings(int count, List<Studio> studios) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      bookings.add(Booking(
        projectId: 1, // Use a default project ID
